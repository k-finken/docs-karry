...
import { FeatureCallout } from '/snippets/feature-callout.mdx';
...
The callout below is an imported component snippet rendered inside a
`for="humans"` block. It appears here on the rendered page, but is stripped from
this page's `.md` export. The `for="agents"` block beneath it does the opposite.

<Visibility for="humans">
  <FeatureCallout name="the Visibility component" />
</Visibility>

<Visibility for="agents">
The Visibility component renders audience-specific content. This sentence is
hidden from human readers but included in the Markdown that AI agents fetch.
</Visibility>

<Note>
  Append `.md` to this page's URL to confirm: the human-only callout (and its
  imported snippet definition) is gone, and the agents-only text is included.
</Note>

## Snippet outside Visibility

The snippet below is rendered in normal page content — not inside any
`Visibility` block. It should appear on both the rendered page and the `.md`
export. Only its `import` line is stripped from the Markdown; the snippet usage
itself stays.
